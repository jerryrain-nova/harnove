...
当前仓库代码。`structure/` 只在需求完成后的总结阶段更新，保存对当前项目功能模块、
代码框架、结构定义和关系的抽象。
...
- 是否残留模板占位符。
- 文档是否具备基本可审核内容。
- 候选 PRD、技术方案和代码方案是否把总览、版本核心差异放在细则前。
...
PRD、技术方案、代码方案和测试方案四个闸门全部批准后，状态机先创建并切换本轮
独立分支，代码专家才能修改业务代码。完成代码修改记录后执行普通 `submit`。
...
`harnove/iterations/`、`harnove/improve/`、`harnove/structure/` 与 `harnove/custom/`；运行时采集 Git 证据时也会自动应用
该排除规则。
...
```powershell
python harnove/scripts/version_policy.py --previous 4.2.0 --current 5.0.0 --change-type architecture
```
