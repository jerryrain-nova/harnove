...
本工具可从现有 PRD 或自然语言需求启动代码迭代，并提供两种互相独立的模式：

- **专家模式（expert，默认）**：需求澄清 → 技术方案 → 代码方案 → 测试方案 →
  代码实现 → 测试执行 → 总结。保留现有完整流程和小改动合并设计能力。
- **敏捷模式（agile）**：需求澄清 → 代码方案 → 代码实现并更新 structure。删除
  独立产品设计、技术方案、测试方案、测试执行和总结阶段。

主 Agent 必须先建议迭代名称并询问用户选择模式。两种模式的每个环节都由
...
`structure/` 的项目抽象内容在专家模式总结阶段或敏捷模式代码实现完成后更新。
...
- Python 3.10 或更高版本。
- 目标项目必须使用 Git；代码实施默认创建并切换独立分支，敏捷模式可由用户明确
  选择当前原分支。
...
`order-batch-export`，并询问使用专家模式还是敏捷模式。等待用户明确选择模式、
采用建议名称或输入新名称后才能调用 `init`。通过 `--mode expert|agile` 和
`--iteration-name` 传入；旧调用省略 `--mode` 时默认使用专家模式。
...
上面是专家模式的完整阶段目录。敏捷模式沿用相同的阶段编号和公共归档结构，但只创建
实际拥有的阶段目录：

```text
harnove/iterations/
└─ 20260720_ITER-20260720-003_order-validation/
   ├─ 00-input/
   ├─ 02-code-plan/
   ├─ 04-implementation/
   ├─ reviews/
...
敏捷模式不会创建空的 `01-technical-design/`、`03-test-design/`、
`05-test-execution/` 或 `06-summary/`，历史迭代目录不会因此被删除或改写。
...
此时状态进入 `awaiting_prd_review`，不会自动进入下一设计阶段。用户必须审核完整
候选 PRD：
...
.\harnove\run.ps1 review `
  ...
  --decision approve `
  --reviewer "产品负责人" `
  --human-confirmation "我已审核完整需求文档，明确批准进入下一设计阶段"
```
...
只有批准版本才会冻结为后续设计及实施的需求基线。原始输入、所有候选版本及用户
补充均保留在 `00-input/` 中。

专家模式批准后进入 `technical_design`；敏捷模式批准后直接进入 `code_plan`。
设计子 Agent 必须实时读取需求相关仓库代码并记录文件或符号证据，不能把
`structure/` 当作架构事实来源。

### 6.3 敏捷模式

敏捷模式可使用现有 PRD 或自然语言输入，只需把初始化参数改为：

```powershell
.\harnove\run.ps1 init `
  --mode agile `
  --iteration-id ITER-20260720-003 `
  --iteration-name order-validation `
  --requirement order-validation `
  --description "沿用现有提示，调整订单模块的局部校验逻辑"
```

敏捷模式生成 PRD 时，只有发现实际模糊点才向用户提问。展示 READY PRD 后只进行
一次正式人工审核；用户明确批准当前 PRD 即表示没有剩余澄清需求，不需要再单独确认
“需求澄清完毕”，也不要求批准文本同时包含两种确认语义。状态机归档该批准效果后
直接进入代码方案。代码方案声明
`DESIGN_MODE: AGILE` 和 `CHANGE_SCOPE: AGILE`，只输出文件、模块、符号和行为级
改动方案，不得修改代码。

代码方案反馈仍先进入变更影响预览；用户批准变化范围后才生成新版本。完整代码方案
获人工批准后，状态机先暂停并要求用户选择直接在当前原分支修改，还是新建分支。
主 Agent 必须主动展示当前分支和建议的新分支名并询问，不能擅自使用默认值。用户
选择归档后才能派发实现。代码实现完成后，同一个实现子 Agent 必须根据完成后的实时代码更新
`structure/`。实现和 structure 更新校验通过后直接完成迭代，不再创建总结阶段，
也不生成总结、improve 或 custom 经验产物。

敏捷模式的迭代目录仅创建 `00-input/`、`02-code-plan/`、`04-implementation/`
三个阶段目录；`reviews/`、`agent-runs/`、`state.json` 等公共归档内容与专家模式
一致。
...
- `awaiting_user_clarification`：候选 PRD 等待用户补齐边界，不能进入当前模式的设计阶段。
...
状态输出中的 `workflow_mode` 展示 `expert|agile`，`timeout_profile` 展示项目规模
判断、历史超时次数及本轮各阶段阈值；专家模式的 `design_mode` 展示代码与测试设计
采用 `separate` 还是 `combined`，敏捷模式显示 `agile`。
...
专家模式会在实施阶段 `dispatch` 生成 work order 前创建并切换新分支。默认名称是
...
敏捷模式代码方案批准后，状态变为 `awaiting_implementation_branch_decision`。
主 Agent 必须把状态中的 `current_branch` 和 `suggested_new_branch` 原样展示给用户，
主动询问使用哪个分支。

用户选择当前原分支：

```powershell
.\harnove\run.ps1 implementation-branch-decision `
  --archive <迭代目录> `
  --strategy current `
  --responder "产品负责人"
```

用户选择新建分支，可以沿用建议名称，也可以指定精确名称：
...
省略 `--branch` 时使用状态中的建议名称。只有选择命令成功后，敏捷模式才允许执行
`dispatch`；分支选择后如果当前检出分支发生变化，实施会停止并要求重新确认。
...
专家模式的技术方案、代码修改方案和测试方案提交后都会进入人工审核状态；敏捷模式
只有需求文档和代码方案两个文档闸门。
...
专家模式下，PRD、技术方案、代码方案和测试方案四个闸门全部批准后，状态机先创建
并切换本轮独立分支；专家合并设计按原有规则减少为一次代码/测试方案审核。敏捷模式
只要求需求和代码方案批准，之后进入完全相同的分支创建与代码实现流程。
...
本节仅适用于专家模式。敏捷模式不会创建测试方案、测试执行或测试修复轮次。
...
专家模式总结子 Agent 必须实时检查完成后的仓库，在同一阶段更新 `harnove/structure/`，
...
敏捷模式没有总结阶段。代码实现子 Agent 在完成代码修改后同步更新
`harnove/structure/`，提交时同样校验 structure 相对实现派发时已经变化，并包含
“功能模块、代码框架、结构定义和关系”。校验成功后迭代直接变为 `complete`。
...
```powershell
python harnove/scripts/version_policy.py --previous 5.3.0 --current 5.4.0 --change-type feature
```
