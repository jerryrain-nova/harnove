...
- **敏捷模式（agile）**：需求澄清 → 代码方案 → 代码实现 → 总结。删除独立产品
  设计、技术方案、测试方案和测试执行阶段。
...
敏捷模式不会创建空的 `01-technical-design/`、`03-test-design/` 或
`05-test-execution/`，历史迭代目录不会因此被删除或改写。
...
获人工批准后，状态机使用与专家模式完全相同的分支创建和代码实现逻辑。实现提交后
跳过测试阶段直接总结，更新 `structure/`、`improve/`、custom 经验，并在
“代码改动点”中向用户展示实际变化。

敏捷模式的迭代目录仅创建 `00-input/`、`02-code-plan/`、`04-implementation/`、
`06-summary/` 四个阶段目录；`reviews/`、`agent-runs/`、`state.json` 等公共归档
内容与专家模式一致。
...
专家模式测试通过后进入总结；敏捷模式代码实现提交后直接进入总结。总结文档必须包含：

- 需求背景和批准范围。
- 实际代码变更。
- 测试结论和剩余风险。
- 需求、方案、代码和测试追溯矩阵。
- 对当前模式实际执行阶段的评分；不得为敏捷模式虚构技术或测试阶段证据。
- 亮点、缺点、根因、经验总结、下次复用规则和改进项。
- 根据本次澄清、审核意见和 custom 更新提炼的“用户反馈经验”。
- 根据完成后的实时仓库代码更新的项目结构抽象。
- 敏捷模式额外包含“代码改动点”，向用户归纳实际文件、符号、行为变化与方案偏差。
