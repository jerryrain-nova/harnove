...
Harnove 是一个面向代码研发的迭代 Harness。它把一次需求从 PRD 整理、技术设计、
代码方案、测试设计、代码实现、测试执行一直推进到迭代总结，并在关键文档阶段保留
人工审核闸门。

Harnove 的主 Agent 只负责流程编排；每个阶段和文档版本都由全新的隔离子 Agent
完成。技术方案和代码方案直接读取当前仓库代码，代码实施前会创建独立 Git 分支，
所有文档、反馈、测试结果和 Git 证据都会保存在项目内的 Harnove 目录中。
...
```text
使用 $harnove 启动 ITER-20260723-001 迭代。
需求名称：order-export。
...
```text
/harnove 启动 ITER-20260723-001，需求名称 order-export：
...
```text
使用 $harnove，基于 docs/prd/order-refund.md 启动 ITER-20260723-002 迭代。
需求名称：order-refund。
```
...
读取代码框架。只有测试通过后的总结阶段，才会把完成后的项目结构抽象写入
`structure/`，供人阅读和后续总结使用。

测试失败时，流程会回到新的代码实施轮次，创建新的实施版本并使用新的子 Agent；
修复后会再次执行测试。所有必测项通过且迭代总结成功归档后，迭代状态才是
`complete`。总结阶段不再设置额外人工审核闸门。
...
最终总结文件位于 `06-summary/`，名称格式为
`<迭代 ID>_<需求名称>_迭代总结_vNNN.md`。Agent 在每个阶段也会返回当前产物的
完整路径。
