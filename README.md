...
完成。方案设计直接读取当前仓库代码，代码实施前会按用户确认选择原分支或新分支，所有文档、
...
```text
需求澄清 → 代码方案 → 代码实现并更新 structure
```

敏捷模式会省略产品方案、技术方案、测试方案、测试执行和总结阶段，但仍遵守 `custom/`
...
敏捷模式生成 PRD 时，只有发现实际模糊点才向用户提问。READY PRD 获用户明确批准
后即可进入代码方案；批准当前 PRD 即表示没有剩余澄清需求，不需要再单独确认
“需求澄清完毕”。
...
敏捷模式的代码方案获批后，Harnove 必须主动询问用户是在当前原分支修改，还是新建
分支实施，并展示当前分支与建议的新分支名。状态机会暂停，用户明确选择并归档后
才能继续派发代码实现；不能根据沉默或默认值替用户决定。选择新分支时可以使用建议
名称，也可以指定其他合法名称。
如果目标分支已经存在或 Git 工作区状态导致无法安全切换，Harnove 会停止并报告，
需要先由用户决定复用、改名或整理工作区。

需求、技术方案和代码方案始终以当前仓库代码为架构依据，不会从 `structure/`
读取代码框架。专家模式在总结阶段更新 `structure/`；敏捷模式在代码实现完成后
直接同步更新 `structure/`。
...
敏捷模式不运行独立测试或总结阶段；代码实现及 `structure/` 更新校验通过后，
迭代直接变为 `complete`。
...
敏捷模式沿用相同的阶段编号和公共归档结构，但只创建 `00-input/`、
`02-code-plan/`、`04-implementation/` 三个阶段目录；不会创建空的技术设计、
测试设计、测试执行或总结目录。`reviews/`、`agent-runs/` 和 `state.json`
与专家模式保持一致。Agent 在每个阶段会返回当前产物的完整路径。
